    ...
    <div style="max-width:640px;margin:0 auto;">
        ...
        @if ($aktifCount >= $batas)
            <div class="alert-bar" style="background:#FEF2F2;border:1px solid #FECACA;color:#DC2626;margin-bottom:16px;">
                ...
                <div>
                    ...
                    <div style="font-size:13px;margin-top:2px;">Anda sudah memiliki {{ $batas }} laporan aktif.
                        Silakan tunggu hingga laporan selesai atau ditolak.</div>
                </div>
            </div>
        @elseif($aktifCount > 0)
            <div class="alert-bar alert-warn" style="margin-bottom:16px;">
                ...
                <div>
                    <div style="font-weight:700;">{{ $aktifCount }} laporan aktif</div>
                    <div style="font-size:13px;margin-top:2px;">Sisa kuota: {{ $batas - $aktifCount }} laporan lagi.</div>
                </div>
            </div>
        @else
            <div class="alert-bar" style="background:#F0FDF4;border:1px solid #BBF7D0;color:#15803D;margin-bottom:16px;">
                ...
                <div style="font-size:13px;">Anda dapat mengirim hingga <strong>{{ $batas }} laporan</strong> secara
                    bersamaan.</div>
            </div>
        @endif
        ...
        <div class="card">
            <div class="card-hd" style="flex-direction:column;align-items:flex-start;gap:2px;">
                ...
                <span style="font-size:12px;color:var(--muted);font-weight:400;">Laporkan kondisi sarana yang perlu
                    diperbaiki</span>
            </div>
            <div class="card-bd">
                @if ($aktifCount >= $batas)
                ...
                @else
                    <form method="POST" action="{{ route('siswa.aspirasi.store') }}" enctype="multipart/form-data">
                        ...
                        <div class="field">
                            <label><span class="msym sz18"
                                    style="vertical-align:middle;margin-right:4px;">grid_view</span>Kategori <span
                        ...
                        <div class="field">
                            <label><span class="msym sz18"
                                    style="vertical-align:middle;margin-right:4px;">meeting_room</span>Ruangan <span
                                    class="req">*</span></label>
                            <select name="ruangan_id" class="sel" required>
                                <option value="">-- Pilih Ruangan --</option>
                                @foreach ($ruangans as $r)
                                    <option value="{{ $r->id_ruangan }}"
                                        {{ old('ruangan_id') == $r->id_ruangan ? 'selected' : '' }}>
                        ...
                        <div class="field">
                            <label><span class="msym sz18"
                                    style="vertical-align:middle;margin-right:4px;">location_on</span>Lokasi <span
                                    class="req">*</span></label>
                            <input type="text" name="lokasi" id="lokasiInp" class="inp" value="{{ old('lokasi') }}"
                                maxlength="50" placeholder="Contoh: Dekat pintu masuk, sudut kiri" required>
                        ...
                        <div class="field">
                            <label><span class="msym sz18"
                                    style="vertical-align:middle;margin-right:4px;">description</span>Keterangan <span
                                    class="req">*</span></label>
                            <textarea name="ket" id="ketInp" class="ta" rows="5" maxlength="255"
                                placeholder="Jelaskan kondisi kerusakan/masalah secara detail..." required>{{ old('ket') }}</textarea>
                        ...
                        <div class="field">
                            <label><span class="msym sz18"
                                    style="vertical-align:middle;margin-right:4px;">photo_camera</span>Foto Laporan <span
                                    style="color:var(--muted);font-weight:400;">(opsional)</span></label>
                            <input type="file" name="foto_laporan" class="inp"
