                ...
                <tbody>
                    @forelse($riwayats as $i => $item)
                        ...
                        <tr>
                            ...
                            <td>
                                @if ($status === 'Selesai' && $asp?->foto_selesai)
                                    <a href="{{ Storage::url($asp->foto_selesai) }}" target="_blank">
                                        <img src="{{ Storage::url($asp->foto_selesai) }}" alt="bukti"
                                            style="width:40px;height:40px;object-fit:cover;border-radius:4px;border:1px solid #e5e7eb;">
                                    </a>
                                @else
                                    <span style="color:var(--muted);font-size:12px;">—</span>
                    ...
                    @empty
                        <tr>
                            <td colspan="9" style="text-align:center;padding:40px;color:var(--muted);">
                                <span class="msym sz28"
                                    style="display:block;margin-bottom:8px;opacity:.4;">receipt_long</span>
                                Belum ada pengaduan
                            </td>
                        </tr>
                    @endforelse
